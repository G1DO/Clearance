...
record observations without changing runner lifecycle state or releasing ownership.
...
- The first `SUCCEEDED` or `FAILED` is sticky. Later progress or a conflicting terminal result
...
A restarted sender must continue that allocation's sequence. Report timestamps and optional
detail/error strings do not determine acceptance and are not persisted in this slice.
All reports preserve `runners.state`, `runners.epoch`, and allocation `ACTIVE` ownership.
Terminal reports therefore leave the runner unavailable for another claim. Cleanup proof and
physical release remain later work.
...
the allocation row, serializing against the scheduler's conditional runner update. Reports
validate authority, sequence, and terminal rules before any `UPDATE`. Poll/report return
